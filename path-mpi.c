           ...
           int* offsets)  
{
    ...
    for (int j = offsets[rank]; j < offsets[rank+1]; ++j) {
        for (int i = 0; i < n; ++i) {
            int lij = l[j*n+i];
            for (int k = 0; k < n; ++k) {
                int lik = l[k*n+i];
            ...
            lnew[(j-offsets[rank])*n+i] = lij;
        }
    }
    return done;
}

int* alloc_partition(int n, int size)
{
    int np = size;
...
void shortest_paths(int n, int* restrict l, int rank, int size)
{
    ...
    int* offsets = alloc_partition(n, size);
    int* restrict lnew = (int*) calloc((offsets[rank+1]-offsets[rank])*n, sizeof(int));
    //memcpy(lnew, l, n*n * sizeof(int));
    int global_done = 1, local_done;
    // Repeated squaring until nothing changes
    do {
        local_done = square(n, l, lnew, rank, offsets);

        MPI_Allgather(lnew, (offsets[rank+1] - offsets[rank])*n, MPI_INT, l, (offsets[rank+1] - offsets[rank])*n, MPI_INT, MPI_COMM_WORLD);
